...
from app.schemas.reservations import RerservationCreateRequest, RerservationCreateResponse
...
async def make_room_reservation(reservation_data: RerservationCreateRequest, db: Session = Depends(get_db)) -> Union[RerservationCreateResponse, Dict[str, str]]:
    valid_reservation = is_reservation_valid(reservation_data=reservation_data, db=db)
    if not valid_reservation:
        return {
            "error": "Not a valid reservation"
        }
...
async def cancel_room_reservation(reservation_id: int, db: Session = Depends(get_db)) -> Dict[str, str]:
