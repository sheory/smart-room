...
async def get_rooms(limit: int, offset: int, db: Session = Depends(get_db)) -> RoomGetAllResponse:
    ...
    logger.info(f"Got {len(rooms)} rooms successfully with pagination: limit={limit}, offset={offset}.")
...
async def check_availability(params: RoomCheckAvailabilityRequest, db: Session = Depends(get_db)) -> bool:
    any_room: RoomModel = (
        db.query(RoomModel)
