...
from app.schemas.reservations import RerservationCreateRequest, RerservationCreateRequest, RerservationCreateResponse
from app.core.logger import logger


def is_reservation_valid(reservation_data: RerservationCreateRequest, db: Session = Depends(get_db)) -> Union[RerservationCreateRequest, Dict[str, str]]:
...
async def make_reservation(reservation_data: RerservationCreateRequest, db: Session = Depends(get_db)) -> Union[RerservationCreateResponse, Dict[str, str]]:
    new_reservation = ReservationModel(**reservation_data.model_dump())

    room = db.get(Room, reservation_data.room_id)

    if not room: #TODO - colocar logica em um lugar q de p reutilizar (no modelo? ou no utils?)
    ...
    return RerservationCreateRequest(**new_reservation.__dict__)


async def cancel_reservation(reservation_id: int, db: Session = Depends(get_db)) -> Dict[str, str]:
