...
def upgrade() -> None:
    ...
    op.create_table('room',
        sa.Column('id', sa.INTEGER(), primary_key=True, autoincrement=True, server_default=sa.text("nextval('room_id_seq')")),
    ...
    op.create_table('reservation',
        sa.Column('id', sa.INTEGER(), autoincrement=True, primary_key=True, server_default=sa.text("nextval('room_id_seq')")), #TODO - precisa mesmo dessa coluna
        sa.Column('user_name', sa.VARCHAR(length=50), nullable=False),#TODO - relacionar user_id a tabela user, se for relacionar copm nome usa unique
